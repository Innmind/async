...
return static function(Prove $prove) {
    ...
    yield $prove
        ->proof('Carry value via the scope')
        ->given(
            Set::type(),
            Set::type(),
        )
        ->test(static function($assert, $initial, $modified) {
    ...
    yield $prove
        ->proof('Retrieve the tasks results')
        ->given(
            Set::type(),
        )
    ...
    yield $prove
        ->proof('Limit concurrency')
        ->given(
            Set::integers()->between(2, 10),
            Set::integers()->between(2, 10),
        )
        ->test(static function($assert, $tasks, $max) {
            $assert
